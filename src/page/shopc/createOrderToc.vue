...
import {
  ...
  export default {
    ...
    methods: {
      ...
      wxPay(data){
        this.cnzzTrackEvent('C端创建订单','支付','订单号：'+data.order_code);
         let that = this
            ...
            //微信支付的一些认证  需要去网站设置好  然后在这调用
                ...
                //这个是支付成功的回调
                    ...
                    // 使用以上方式判断前端返回,微信团队郑重提示：
                            ...
                            // alert(1)
                            // alert(res.err_msg)
                            that.cnzzTrackEvent('C端创建订单','支付成功','订单号：'+data.order_code);
                            that.$router.push({
                                path:'/orderRusult'
                    ...
                    //else 支付不成功的回调
                        ...
                        // alert(2)
                        // alert(res.err_msg)
                        that.$router.push({path: '/orderToC/'+data.order_code,query: {distributor_id:that.$route.query.distributor_id}})
                    }
                }); 
      }
    },
